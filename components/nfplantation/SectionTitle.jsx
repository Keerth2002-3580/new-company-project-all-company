...
const SectionTitle = ({ title, subtitle, center = true, light = false }) => {
    return (
        <div className={`mb-12 ${center ? 'text-center' : 'text-left'} max-w-4xl mx-auto relative z-10`}>
            ...
            <h2 className={`text-3xl lg:text-4xl font-bold mb-4 ${light ? 'text-white' : 'text-white'} tracking-tight`}>
                {title.split(' ').map((word, i) => (
                    <span key={i} className={i === 1 ? "text-transparent bg-clip-text bg-gradient-to-r from-white to-gray-400" : ""}>{word} </span>
                ))}
            </h2>
            {subtitle && <p className={`text-sm ${light ? 'text-gray-300' : 'text-gray-400'} max-w-2xl mx-auto leading-relaxed`}>{subtitle}</p>}
        </div>
    );
};
